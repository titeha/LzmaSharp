...
public interface IFileSystemBrowser
{
  ...
  /// <summary>
  /// Нормализует введённый путь и возвращает канонический полный путь, ЕСЛИ это существующая папка
  /// (для адресной строки — переход по введённому пути); иначе <see langword="null"/>. Реализация по
  /// умолчанию — <see langword="null"/> (шов подключается только в боевой службе).
  /// </summary>
  string? ResolveDirectory(string path) => null;
