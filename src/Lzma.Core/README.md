...
- Отдельный член **любого размера** (в т.ч. >2 ГиБ) — и извлечение, и создание идут потоком
  (потоковый Deflate, ZIP64-размеры при ≥4 ГиБ). Исключение: **шифрование** члена >2 ГиБ пока не
  поддержано (нужен потоковый AES).
