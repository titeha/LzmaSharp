...
- запись AES-архивов;
- хранение паролей; интеграция с системными хранилищами секретов;
...
UI для ввода пароля реализован в этапе 4 (Avalonia) — открытие AES- и ГОСТ-архивов
по паролю работает (см. [`STAGE4_UI_STATUS.md`](STAGE4_UI_STATUS.md)).

### Известное ограничение: AES + сложные цепочки кодеков

Покрыта и работает связка **AES + LZMA2** (single / multi / solid, mhe off/on).
**Не поддерживается** AES поверх сложных цепочек, прежде всего **BCJ2** (фильтр 7-Zip
для исполняемых файлов) — при архивации `.exe`/бинарников 7-Zip включает BCJ2
автоматически. На таком архиве расшифровка проходит, но раскодирование цепочки
возвращает `NotSupported` (а на некоторых формах — `InvalidData`). Поддержка
AES+BCJ2 и других сложных цепочек под шифрованием — отдельная работа по декодеру
(нужны эталонные тест-архивы). Обычные текстовые/документные AES-архивы (AES+LZMA2)
открываются штатно.
...
Этап 1.5 закрыт после финальной сверки AES-контракта, тестов и документации.
...
1. AES-контракт зафиксирован в документации и тестах.
