...
Точка и пробел внутри имени разрешены.
...
К недопустимым Windows-символам относятся:
...
Символы `/` и `\` запрещены отдельно, потому что текущий writer поддерживает только простые имена entry без вложенных путей.
