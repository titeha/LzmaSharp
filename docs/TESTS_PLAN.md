...
- имена entry с пробелом внутри разрешены;
- директория с whitespace-only именем возвращает `InvalidData`;
...
- файл и директория с одинаковым именем возвращают `InvalidData`;
- имена entry, отличающиеся только регистром, возвращают `InvalidData`;
- файл и директория с именами, отличающимися только регистром, возвращают `InvalidData`;
...
- директория с зарезервированным Windows-именем возвращает `InvalidData`;
- похожие, но не зарезервированные имена entry разрешены- имена entry с завершающей точкой возвращают `InvalidData`;
...
- безопасные символы вроде `-`, `_`, `[`, `]` и `.` внутри имени разрешены.
