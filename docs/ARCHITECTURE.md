...
Промежуточный итог writer-а: writer поддерживает простые валидные entry, включая безопасные вложенные `/`-paths.
...
- не содержать `\0`;
- не содержать `\`;
- использовать `/` только как разделитель вложенного entry path.
...
## Контракт вложенных writer entry path
...
Каждый сегмент path проходит тот же контракт имени entry: зарезервированные Windows-имена, недопустимые символы, управляющие символы, завершающая точка и завершающий пробельный символ запрещены.

Если parent-entry существует как директория, вложенный entry внутри неё разрешён.
...
Полные path, отличающиеся только регистром, возвращают `InvalidData`:
