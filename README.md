...
Writer отклоняет некорректные entry path как `InvalidData`.
...
Имя каждого path-сегмента должно быть:
...
Символ `/` разрешён только как разделитель вложенного entry path.
...
Первый реализованный writer-контур:
