...
void Main()
{
	...
	string archivePath = Path.Combine(outputDir, "bzip2_singlefile_mhc_off.7z");
	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharp_BZip2_Single_NoHeader_" + Guid.NewGuid().ToString("N"));

	Directory.CreateDirectory(workDir);

	try
	{
		// Делаем входной файл: 32 KiB байт 'A'.
		WriteFilledFile(Path.Combine(workDir, "bzip2.bin"), 32 * 1024, 0x41);
		...
		psi.ArgumentList.Add("bzip2.bin");
