...
void Main()
{
	...
	string archivePath = Path.Combine(outputDir, "swap2_deflate64_mhc.7z");

	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharp_Swap2_Deflate64_" + Guid.NewGuid().ToString("N"));
	...
	try
	{
		...
		File.WriteAllBytes(Path.Combine(workDir, "swap2_deflate64.bin"), input);
		...
		psi.ArgumentList.Add("swap2_deflate64.bin");
		psi.ArgumentList.Add("-t7z");

		// Цепочка: Swap2 -> Deflate64
