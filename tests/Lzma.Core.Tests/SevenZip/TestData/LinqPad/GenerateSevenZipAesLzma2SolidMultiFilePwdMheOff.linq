...
// tests/Lzma.Core.Tests/SevenZip/TestData/Real/aes_lzma2_solid_multifile_pwd_mhe_off.7z
//
// Назначение:
// реальный solid 7z AES-архив без шифрования заголовков,
// несколько файлов, LZMA2, пароль LzmaSharp-AES-Stage15.
//
// Используется тестом:
// SevenZipReal7zAesLzma2SolidMultiFileTests
...
void Main()
{
	...
	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharpAesLzma2SolidMultiFileRealArchive",
	...
	try
	{
		string archivePath = Path.Combine(realDir, "aes_lzma2_solid_multifile_pwd_mhe_off.7z");

		File.WriteAllBytes(
			Path.Combine(inputRoot, "alpha.txt"),
			Encoding.UTF8.GetBytes(
				"alpha alpha alpha alpha alpha\r\n"
			  + "solid AES LZMA2 test payload A\r\n"));

		File.WriteAllBytes(
			Path.Combine(nestedDir, "beta.txt"),
			Encoding.UTF8.GetBytes(
