...
void Main()
{
	...
	string archivePath = Path.Combine(outputDir, "bcj_x86_bzip2_mhc.7z");

	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharp_BCJx86_BZip2_Single_" + Guid.NewGuid().ToString("N"));
	...
	try
	{
		...
		File.WriteAllBytes(Path.Combine(workDir, "x86_bzip2.bin"), fileBytes);
		...
		psi.ArgumentList.Add("x86_bzip2.bin");

		psi.ArgumentList.Add("-t7z");

		// Явно задаём цепочку: BCJ(x86) -> BZip2
		psi.ArgumentList.Add("-m0=BCJ");
		psi.ArgumentList.Add("-m1=BZip2");

		// Header compressed (EncodedHeader).
