...
void Main()
{
	...
	string archivePath = Path.Combine(outputDir, "swap2_deflate_mhc.7z");

	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharp_Swap2_Deflate_" + Guid.NewGuid().ToString("N"));

	Directory.CreateDirectory(workDir);

	try
	{
		// Данные: big-endian ramp из UInt16, чтобы Swap2 имел смысл как фильтр.
		byte[] input = CreateU16BigEndianRamp(sampleCount: 16 * 1024); // 32768 байт
		File.WriteAllBytes(Path.Combine(workDir, "swap2.bin"), input);
		...
		psi.ArgumentList.Add("swap2.bin");
		psi.ArgumentList.Add("-t7z");

		// Цепочка: Swap2 -> Deflate
