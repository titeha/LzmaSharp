...
void Main()
{
	...
	string archivePath = Path.Combine(outputDir, "bcj_x86_deflate_mhc.7z");

	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharp_BCJx86_Deflate_Single_" + Guid.NewGuid().ToString("N"));
	...
	try
	{
		...
		// Явно задаём цепочку: BCJ(x86) -> Deflate
		psi.ArgumentList.Add("-m0=BCJ");
		psi.ArgumentList.Add("-m1=Deflate");

		// Header compressed (EncodedHeader) — для консистентности с большинством real тестов.
