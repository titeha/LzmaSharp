...
void Main()
{
	...
	string archivePath = Path.Combine(outputDir, "swap2_ppmd_mhc.7z");

	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharp_Swap2_PPMd_" + Guid.NewGuid().ToString("N"));
	...
	try
	{
		...
		File.WriteAllBytes(Path.Combine(workDir, "swap2_ppmd.bin"), input);
		...
		psi.ArgumentList.Add("swap2_ppmd.bin");
