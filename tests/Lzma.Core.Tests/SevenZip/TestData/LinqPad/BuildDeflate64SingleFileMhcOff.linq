...
void Main()
{
	...
	string archivePath = Path.Combine(outputDir, "deflate64_singlefile_mhc_off.7z");

	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharp_Deflate64_Single_NoHeader_" + Guid.NewGuid().ToString("N"));

	Directory.CreateDirectory(workDir);

	try
	{
		WriteFilledFile(Path.Combine(workDir, "deflate64.bin"), 16 * 1024, 0x41);
		...
		psi.ArgumentList.Add("deflate64.bin");

		psi.ArgumentList.Add("-t7z");
		psi.ArgumentList.Add("-m0=Deflate64");
