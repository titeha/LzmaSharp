...
void Main()
{
	...
	string archivePath = Path.Combine(outputDir, "hello_copy_7zaes_mhc_off.7z");

	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharp_Hello_Copy_7zAes_NoHeader_" + Guid.NewGuid().ToString("N"));
	...
	try
	{
		...
		// Пароль на данные, но header оставляем открытым.
		psi.ArgumentList.Add("-psecret");
		psi.ArgumentList.Add("-mhe=off");

		// И сам header не сжимаем, чтобы получить plain Header.
