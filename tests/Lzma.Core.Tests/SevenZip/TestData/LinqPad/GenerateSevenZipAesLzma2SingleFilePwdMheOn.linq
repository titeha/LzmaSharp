...
// tests/Lzma.Core.Tests/SevenZip/TestData/Real/aes_lzma2_singlefile_pwd_mhe_on.7z
//
// Назначение:
// реальный 7z AES-архив с зашифрованным header,
// один файл, LZMA2, пароль LzmaSharp-AES-Stage15.
//
// Используется тестом:
// SevenZipReal7zAesEncryptedHeaderTests
...
void Main()
{
	...
	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharpAesEncryptedHeaderRealArchive",
	...
	try
	{
		string inputPath = Path.Combine(workDir, "aes-mhe-on-real.txt");
		string archivePath = Path.Combine(realDir, "aes_lzma2_singlefile_pwd_mhe_on.7z");

		File.WriteAllBytes(
			inputPath,
			Encoding.UTF8.GetBytes("LzmaSharp AES encrypted header real 7z test\r\n"));
