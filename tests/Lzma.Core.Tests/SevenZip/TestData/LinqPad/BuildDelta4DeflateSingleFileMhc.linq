...
void Main()
{
	...
	string outputDir = Path.Combine(repoRoot, "tests", "Lzma.Core.Tests", "SevenZip", "TestData", "Real");
	Directory.CreateDirectory(outputDir);

	string archivePath = Path.Combine(outputDir, "delta4_deflate_mhc.7z");

	string workDir = Path.Combine(Path.GetTempPath(), "LzmaSharp_Delta4_Deflate_" + Guid.NewGuid().ToString("N"));
	Directory.CreateDirectory(workDir);

	try
	{
		byte[] input = CreateStereo16SamplesBytes(sampleCount: 16 * 1024); // 65536 байт
		File.WriteAllBytes(Path.Combine(workDir, "delta4.bin"), input);
		...
		psi.ArgumentList.Add("delta4.bin");
		psi.ArgumentList.Add("-t7z");

		// Цепочка: Delta(offset=4) -> Deflate
		// В документации 7-Zip пример: "0=Delta:4" (offset в байтах).
		...
		new { psi = psi.FileName, args = string.Join(" ", psi.ArgumentList), p.ExitCode, stdout, stderr }.Dump("7z");
