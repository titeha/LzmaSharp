...
void Main()
{
	...
	string archivePath = Path.Combine(outputDir, "swap4_ppmd_mhc.7z");

	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharp_Swap4_PPMd_" + Guid.NewGuid().ToString("N"));

	Directory.CreateDirectory(workDir);

	try
	{
		// Данные: big-endian ramp из UInt32, чтобы Swap4 был осмысленным фильтром.
		// 16384 * 4 = 65536 байт.
		byte[] input = CreateU32BigEndianRamp(sampleCount: 16 * 1024);
		File.WriteAllBytes(Path.Combine(workDir, "swap4_ppmd.bin"), input);
		...
		psi.ArgumentList.Add("swap4_ppmd.bin");
