...
void Main()
{
	...
	string archivePath = Path.Combine(outputDir, "swap4_bzip2_mhc.7z");

	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharp_Swap4_BZip2_" + Guid.NewGuid().ToString("N"));
	...
	try
	{
		...
		File.WriteAllBytes(Path.Combine(workDir, "swap4_bzip2.bin"), input);
		...
		psi.ArgumentList.Add("swap4_bzip2.bin");
