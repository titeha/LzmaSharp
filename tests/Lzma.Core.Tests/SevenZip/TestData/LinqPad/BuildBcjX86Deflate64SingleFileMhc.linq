...
void Main()
{
	...
	string archivePath = Path.Combine(outputDir, "bcj_x86_deflate64_mhc.7z");

	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharp_BCJx86_Deflate64_Single_" + Guid.NewGuid().ToString("N"));
	...
	try
	{
		...
		// Явно задаём цепочку: BCJ(x86) -> Deflate64
