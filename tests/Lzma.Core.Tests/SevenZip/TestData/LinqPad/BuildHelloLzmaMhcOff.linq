...
void Main()
{
	...
	string archivePath = Path.Combine(outputDir, "hello_lzma_d1m_mhc_off.7z");

	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharp_Lzma_Single_NoHeader_" + Guid.NewGuid().ToString("N"));

	Directory.CreateDirectory(workDir);

	try
	{
		// Делаем входной файл как в уже существующем hello_lzma_d1m_mhc.7z:
		// 16 KiB байт 'A' => hello.bin.
		...
		// Явно фиксируем LZMA (не LZMA2) и словарь 1 MiB.
		psi.ArgumentList.Add("-m0=LZMA:d=1m");
	...
	finally
	{
		...
		{
			// Если ОС держит хэндл — удалишь temp вручную позже.
		}
	}
}
