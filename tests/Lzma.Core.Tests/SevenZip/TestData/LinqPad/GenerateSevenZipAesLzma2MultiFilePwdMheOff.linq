...
// tests/Lzma.Core.Tests/SevenZip/TestData/Real/aes_lzma2_multifile_pwd_mhe_off.7z
//
// Назначение:
...
//
// Используется тестом:
// SevenZipReal7zAesLzma2MultiFileTests
...
void Main()
{
	...
	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharpAesLzma2MultiFileRealArchive",
	...
	try
	{
		string archivePath = Path.Combine(realDir, "aes_lzma2_multifile_pwd_mhe_off.7z");

		File.WriteAllBytes(
			Path.Combine(inputRoot, "file-one.txt"),
			Encoding.UTF8.GetBytes("LzmaSharp AES LZMA2 multi-file test: file one\r\n"));
