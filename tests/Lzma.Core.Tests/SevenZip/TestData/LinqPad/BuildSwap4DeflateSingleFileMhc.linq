...
void Main()
{
	...
	string archivePath = Path.Combine(outputDir, "swap4_deflate_mhc.7z");

	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharp_Swap4_Deflate_" + Guid.NewGuid().ToString("N"));
	...
	try
	{
		...
		File.WriteAllBytes(Path.Combine(workDir, "swap4.bin"), input);
		...
		psi.ArgumentList.Add("swap4.bin");
		psi.ArgumentList.Add("-t7z");

		// Цепочка: Swap4 -> Deflate
