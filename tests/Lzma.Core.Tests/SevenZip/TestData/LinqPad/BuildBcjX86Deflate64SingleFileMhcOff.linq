...
void Main()
{
	...
	string archivePath = Path.Combine(outputDir, "bcj_x86_deflate64_mhc_off.7z");

	string workDir = Path.Combine(
		Path.GetTempPath(),
		"LzmaSharp_BCJx86_Deflate64_Single_NoHeader_" + Guid.NewGuid().ToString("N"));
	...
	try
	{
		...
		File.WriteAllBytes(Path.Combine(workDir, "x86_deflate64.bin"), fileBytes);
		...
		psi.ArgumentList.Add("x86_deflate64.bin");

		psi.ArgumentList.Add("-t7z");

		// Цепочка: BCJ(x86) -> Deflate64
