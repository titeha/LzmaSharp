...
public sealed class SevenZipReal7zAesLzma2SolidMultiFileTests
{
  private const string Password = "LzmaSharp-AES-Stage15";

  [Fact]
  public void DecodeToEntries_РеальныйAesLzma2SolidMultiFileАрхив_СПаролем_ВозвращаетФайлы()
  {
    byte[] archive = ReadTestDataBytes("TestData/Real/aes_lzma2_solid_multifile_pwd_mhe_off.7z");
  ...
  [Fact]
  public void DecodeToEntries_РеальныйAesLzma2SolidMultiFileАрхив_БезПароля_ВозвращаетNotSupported()
  {
    byte[] archive = ReadTestDataBytes("TestData/Real/aes_lzma2_solid_multifile_pwd_mhe_off.7z");
  ...
  [Fact]
  public void DecodeToEntries_РеальныйAesLzma2SolidMultiFileАрхив_СНевернымПаролем_ВозвращаетInvalidData()
  {
    byte[] archive = ReadTestDataBytes("TestData/Real/aes_lzma2_solid_multifile_pwd_mhe_off.7z");
  ...
  [Fact]
  public void ExtractToDirectory_РеальныйAesLzma2SolidMultiFileАрхив_СПаролем_ЗаписываетФайлы()
  {
    byte[] archive = ReadTestDataBytes("TestData/Real/aes_lzma2_solid_multifile_pwd_mhe_off.7z");
  ...
  private static Dictionary<string, byte[]> CreateExpectedFiles()
  {
    return new Dictionary<string, byte[]>(StringComparer.Ordinal)
    {
      ["alpha.txt"] = Encoding.UTF8.GetBytes(
        ...
        + "solid AES LZMA2 test payload A\r\n"),

      ["nested/beta.txt"] = Encoding.UTF8.GetBytes(
          "beta beta beta beta beta\r\n"
        + "solid AES LZMA2 test payload B\r\n"),
  ...
  private static string CreateTempRoot()
  {
    return Path.Combine(
        Path.GetTempPath(),
        "LzmaSharpTests",
        nameof(SevenZipReal7zAesLzma2SolidMultiFileTests),
