...
public sealed class SevenZipReal7zBcjX86Bzip2HeaderNotEncodedTests
{
  [Fact]
  public void DecodeToArray_Real7z_BcjX86_BZip2_HeaderNotEncoded_Ok()
  {
    byte[] archive = ReadTestDataBytes("TestData/Real/bcj_x86_bzip2_mhc_off.7z");
    ...
    Assert.Single(files);
    Assert.Equal("x86_bzip2.bin", files[0].Name.Replace('\\', '/'));
