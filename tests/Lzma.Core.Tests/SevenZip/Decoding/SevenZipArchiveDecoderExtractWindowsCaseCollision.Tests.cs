...
public sealed class SevenZipArchiveDecoderExtractWindowsCaseCollisionTests
{
  [Theory]
  [InlineData(false)]
  [InlineData(true)]
  public void ExtractToDirectory_TwoEntries_DifferOnlyByCase_Windows_InvalidData_AndNothingExtracted(bool overwrite)
  {
    if (!OperatingSystem.IsWindows())
      return;

    byte[] bytesA = MakePattern(64, mul: 17, add: 3);
    byte[] bytesB = MakePattern(64, mul: 29, add: 5);
    ...
    Assert.Equal("A.bin", entries[0].Name);
    Assert.Equal("a.bin", entries[1].Name);
    Assert.Equal(bytesA, entries[0].Bytes);
    Assert.Equal(bytesB, entries[1].Bytes);

    string root = Path.Combine(
        Path.GetTempPath(),
        "LzmaSharpTests",
        nameof(SevenZipArchiveDecoderExtractWindowsCaseCollisionTests),
    ...
    try
    {
      ...
      // После фикса хотим отказывать ДО начала записи.
