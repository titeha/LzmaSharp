...
public sealed class SevenZipFolderDecoderBcj2ToArrayTests
{
  ...
  [Fact]
  public void TryDecodeBcj2ToArray_JccПриBit1ИУсеченномRel32_ВозвращаетOkСУсеченнымВыходом()
  {
    byte[] buf0 = [0x0F, 0x80];
    ...
    // поэтому первая вероятностная развилка идёт в BIT=1.
    //
    // Для Jcc helper берёт ABS disp32 из buf2.
    // На момент пересчёта outPos уже равен 2:
    //
    //   rel32 = abs - (outPos + 4) = 10 - 6 = 4
    //
    ...
    //   0F 80 04 00 00 00
    //
    // Но outSize = 4, поэтому helper должен вернуть только:
    //   0F 80 04 00
    //
    // Это фиксирует текущий контракт: outSize контейнера ограничивает
    // итоговый output даже внутри четырёхбайтового rel32.
  ...
  [Fact]
  public void TryDecodeBcj2ToArray_0FБезJcc_КопируетБайтыКакОбычные()
  {
    byte[] buf0 = [0x0F, 0x70, 0x11, 0x22];

    // 0F 70 не является Jcc, потому что Jcc в BCJ2 распознаётся
    // только как 0F 8x. Поэтому helper должен просто скопировать
    // все байты из buf0 и не трогать buf1/buf2.
    SevenZipFolderDecodeResult result = SevenZipFolderDecoder.TryDecodeBcj2ToArray(
        buf0: buf0,
        buf1: [0xAA, 0xBB, 0xCC, 0xDD],
        buf2: [0xEE, 0xFF, 0x00, 0x11],
  ...
  [Fact]
  public void TryDecodeBcj2ToArray_Одиночный0FВКонцеBuf0_КопируетКакОбычныйБайт()
  {
    byte[] buf0 = [0x0F];

    // 0F сам по себе ещё не Jcc.
    // Jcc в BCJ2 распознаётся только как пара 0F 8x.
    //
    // Здесь outSize = 1, поэтому helper должен просто скопировать 0F,
    // не пытаясь читать второй байт из buf0.
    SevenZipFolderDecodeResult result = SevenZipFolderDecoder.TryDecodeBcj2ToArray(
        buf0: buf0,
        buf1: [0xAA, 0xBB, 0xCC, 0xDD],
        buf2: [0xEE, 0xFF, 0x00, 0x11],
  ...
  [InlineData((byte)0xE8)]
  [InlineData((byte)0xE9)]
  public void TryDecodeBcj2ToArray_E8ИлиE9ПриBit0ИНедостаткеBuf0_ВозвращаетInvalidData(byte opcode)
  {
    byte[] buf0 = [opcode];

    // Пять нулей => code = 0 после инициализации range decoder,
    // поэтому вероятностная развилка идёт в BIT=0.
    //
    // При BIT=0 helper не берёт disp32 из buf1/buf2:
    // displacement должен остаться в buf0.
    //
    // Но в buf0 есть только opcode, а outSize требует opcode + 4 байта disp32.
    // Поэтому helper должен вернуть InvalidData из финальной проверки outPos != outSize.
    SevenZipFolderDecodeResult result = SevenZipFolderDecoder.TryDecodeBcj2ToArray(
        buf0: buf0,
        buf1: [0xAA, 0xBB, 0xCC, 0xDD],
        buf2: [0xEE, 0xFF, 0x00, 0x11],
  ...
  [Fact]
  public void TryDecodeBcj2ToArray_JccПриBit0ИНедостаткеBuf0_ВозвращаетInvalidData()
  {
    byte[] buf0 = [0x0F, 0x80];
    ...
    // поэтому вероятностная развилка идёт в BIT=0.
    //
    // 0F 80 распознаётся как Jcc.
    // При BIT=0 helper не берёт disp32 из buf2:
    // displacement должен остаться в buf0.
    //
    // Но в buf0 есть только двухбайтовый opcode, а outSize требует
    // opcode + 4 байта disp32. Поэтому helper должен вернуть InvalidData
    // из финальной проверки outPos != outSize.
    SevenZipFolderDecodeResult result = SevenZipFolderDecoder.TryDecodeBcj2ToArray(
        buf0: buf0,
        buf1: [0xAA, 0xBB, 0xCC, 0xDD],
        buf2: [0xEE, 0xFF, 0x00, 0x11],
