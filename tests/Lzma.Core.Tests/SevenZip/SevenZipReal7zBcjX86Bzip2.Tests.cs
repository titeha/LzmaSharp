...
public sealed class SevenZipReal7zBcjX86Bzip2Tests
{
  [Fact]
  public void DecodeToArray_Real7z_7Zip_BcjX86_BZip2_Ok()
  {
    byte[] archive = ReadTestDataBytes("TestData/Real/bcj_x86_bzip2_mhc.7z");

    // 1) Проверяем, что в header реально есть BCJ + BZip2.
    ...
    Assert.Single(files);
    Assert.EndsWith("x86_bzip2.bin", files[0].Name, StringComparison.Ordinal);
