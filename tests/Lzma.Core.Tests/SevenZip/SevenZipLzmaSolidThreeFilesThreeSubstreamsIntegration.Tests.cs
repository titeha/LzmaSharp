...
public sealed class SevenZipLzmaSolidThreeFilesThreeSubstreamsIntegrationTests
{
  [Fact]
  public void DecodeToArray_Lzma1Solid_ThreeFiles_ThreeSubStreams_SizesTwoEntries_Ok()
  {
    const string name0 = "a.bin";
    const string name1 = "b.bin";
    const string name2 = "c.bin";
    ...
    byte[] file2 = new byte[19];
    for (int i = 0; i < file2.Length; i++)
      file2[i] = (byte)(i * 13 + 5);

    // Solid-поток = file0 + file1 + file2 (три НЕ-пустых файла в одном folder).
    byte[] solid = new byte[file0.Length + file1.Length + file2.Length];
    Buffer.BlockCopy(file0, 0, solid, 0, file0.Length);
    Buffer.BlockCopy(file1, 0, solid, file0.Length, file1.Length);
    Buffer.BlockCopy(file2, 0, solid, file0.Length + file1.Length, file2.Length);
    ...
    byte[] archive = Build7z_SolidLzma1_ThreeFiles_ThreeSubStreams(
    ...
    Assert.Equal(file2, files[2].Bytes);
  }

  private static byte[] Build7z_SolidLzma1_ThreeFiles_ThreeSubStreams(
    byte[] packedStream,
    int unpackSizeTotal,
    int size0,
    int size1,
    ...
    int dictionarySize)
  {
    ...
    ArgumentOutOfRangeException.ThrowIfLessThan(unpackSizeTotal, size0 + size1);

    byte[] nextHeader = BuildNextHeader_SolidLzma1_ThreeFiles_ThreeSubStreams(
  ...
  private static byte[] BuildNextHeader_SolidLzma1_ThreeFiles_ThreeSubStreams(
    int packSize,
    int unpackSizeTotal,
    int size0,
    int size1,
    ...
    int dictionarySize)
  {
    ...
    // SubStreamsInfo: 3 sub-streams (3 непустых файла) в одном folder.
    h.Add(SevenZipNid.SubStreamsInfo);

    h.Add(SevenZipNid.NumUnpackStream);
    WriteU64(h, 3); // для единственного folder'а

    h.Add(SevenZipNid.Size);
    // В формате 7z для streams=3 пишется только (streams-1)=2 размера.
    WriteU64(h, (ulong)size0);
    WriteU64(h, (ulong)size1);
