...
public sealed class SevenZipReal7zSwap4Bzip2Tests
{
  [Fact]
  public void DecodeToArray_Real7z_Swap4_BZip2_Ok()
  {
    byte[] archive = ReadTestDataBytes("TestData/Real/swap4_bzip2_mhc.7z");
    ...
    // NextHeaderKind не фиксируем: на маленьких/простых архивах 7-Zip иногда оставляет Header несжатым.
    Assert.True(
    ...
    Assert.Single(files);
    Assert.Equal("swap4_bzip2.bin", files[0].Name.Replace('\\', '/'));
