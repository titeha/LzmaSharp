...
public sealed class SevenZipArchiveDecoderExtractUnsafePathTests
{
  public static IEnumerable<object[]> InvalidEntryNames()
  {
    // Родительский сегмент.
    yield return ["../evil.bin"];
    yield return ["..\\evil.bin"];

    // Абсолютный путь.
    yield return ["/evil.bin"];

    // Запрещённые сегменты.
    yield return ["./evil.bin"];
    yield return ["dir//evil.bin"];

    // Windows-специфика: "C:" и ADS.
    if (OperatingSystem.IsWindows())
    {
      yield return ["C:/evil.bin"];
      yield return ["file.bin:ads"];
    }
  }

  [Theory]
  [MemberData(nameof(InvalidEntryNames))]
  public void ExtractToDirectory_InvalidEntryName_InvalidData_AndNothingExtracted(string entryName)
  {
    ...
    string root = Path.Combine(
        Path.GetTempPath(),
        "LzmaSharpTests",
        nameof(SevenZipArchiveDecoderExtractUnsafePathTests),
    ...
    try
    {
      ...
      // Корневую папку код создаёт заранее, но внутрь ничего попасть не должно.
  ...
  private static byte[] BuildNextHeaderSingleFileCopy(int packSize, int unpackSize, string fileName)
  {
    ...
    h.Add(0x01);      // mainByte: idSize=1, без props, простой coder
