...
public sealed class SevenZipReal7zSwap2Deflate64Tests
{
  [Fact]
  public void DecodeToArray_Real7z_Swap2_Deflate64_Ok()
  {
    byte[] archive = ReadTestDataBytes("TestData/Real/swap2_deflate64_mhc.7z");
    ...
    Assert.Single(files);
    Assert.Equal("swap2_deflate64.bin", files[0].Name.Replace('\\', '/'));
