...
public sealed class SevenZipLzmaSolidTwoFilesIntegrationTests
{
  [Fact]
  public void DecodeToArray_Lzma1Solid_TwoFiles_SplitBySubStreamsInfoSize_Ok()
  {
    ...
    byte[] archive = Build7z_SolidLzma1_TwoFiles(
      packedStream: packedStream,
      unpackSizeTotal: solid.Length,
      firstFileSize: file0.Length,
      fileName0: name0,
      fileName1: name1,
  ...
  private static byte[] Build7z_SolidLzma1_TwoFiles(
    byte[] packedStream,
    int unpackSizeTotal,
    int firstFileSize,
    string fileName0,
    ...
    int dictionarySize)
  {
    byte[] nextHeader = BuildNextHeader_SolidLzma1_TwoFiles(
  ...
  private static byte[] BuildNextHeader_SolidLzma1_TwoFiles(
    int packSize,
    int unpackSizeTotal,
    int firstFileSize,
    string fileName0,
    ...
    int dictionarySize)
  {
    if (firstFileSize < 0 || firstFileSize > unpackSizeTotal)
      throw new ArgumentOutOfRangeException(nameof(firstFileSize));
    ...
    // coder0: LZMA (7z) methodId = {03 01 01}, props = 5 байт:
    ...
    // SubStreamsInfo: 2 sub-streams в одном folder.
    ...
    h.Add(SevenZipNid.Size);
    // В формате 7z для streams=2 пишется только (streams-1)=1 размер.
    WriteU64(h, (ulong)firstFileSize);
    ...
    byte[] nameBytes = Encoding.Unicode.GetBytes(fileName0 + "\0" + fileName1 + "\0");
