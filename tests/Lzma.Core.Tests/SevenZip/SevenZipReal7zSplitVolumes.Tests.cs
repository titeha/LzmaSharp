...
public sealed class SevenZipReal7zSplitVolumesTests
{
  [Fact]
  public void ArchiveReader_Real7z_FirstSplitVolume_NeedMoreInput()
  {
    byte[] part1 = ReadTestDataBytes("TestData/Real/hello_copy_split_v10k_mhc_off.7z.001");

    var reader = new SevenZipArchiveReader();
    SevenZipArchiveReadResult r = reader.Read(part1, out int bytesConsumed);

    Assert.Equal(SevenZipArchiveReadResult.NeedMoreInput, r);
    Assert.InRange(bytesConsumed, 0, part1.Length);

    Assert.False(reader.Header.HasValue);
  }

  [Fact]
  public void DecodeToArray_Real7z_FirstSplitVolume_NeedMoreData()
  {
    byte[] part1 = ReadTestDataBytes("TestData/Real/hello_copy_split_v10k_mhc_off.7z.001");
  ...
  [Fact]
  public void DecodeToArray_Real7z_SplitVolumesConcatenated_Ok()
  {
