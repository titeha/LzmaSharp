...
public sealed class SevenZipReal7zBcjX86LzmaHeaderNotEncodedTests
{
  [Fact]
  public void DecodeToArray_Real7z_BcjX86_Lzma_HeaderNotEncoded_Ok()
  {
    byte[] archive = ReadTestDataBytes("../TestData/Real/bcj_x86_lzma_d1m_mhc_off.7z");
    ...
    var lzma = Array.Find(folder.Coders, c => IsLzma(c.MethodId));
    Assert.NotNull(lzma!.Properties);
    Assert.Equal(5, lzma.Properties!.Length);
    ...
    Assert.Single(files);
    Assert.Equal("x86.bin", files[0].Name.Replace('\\', '/'));
