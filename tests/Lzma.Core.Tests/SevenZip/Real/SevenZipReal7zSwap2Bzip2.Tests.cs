...
public sealed class SevenZipReal7zSwap2Bzip2Tests
{
  [Fact]
  public void DecodeToArray_Real7z_Swap2_BZip2_Ok()
  {
    byte[] archive = ReadTestDataBytes("../TestData/Real/swap2_bzip2_mhc.7z");
    ...
    Assert.Single(files);
    Assert.Equal("swap2_bzip2.bin", files[0].Name.Replace('\\', '/'));
