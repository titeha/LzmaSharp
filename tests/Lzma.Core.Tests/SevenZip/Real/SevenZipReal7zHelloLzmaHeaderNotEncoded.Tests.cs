...
public sealed class SevenZipReal7zHelloLzmaHeaderNotEncodedTests
{
  [Fact]
  public void DecodeToArray_Real7z_Lzma_HeaderNotEncoded_Ok()
  {
    byte[] archive = ReadTestDataBytes("../TestData/Real/hello_lzma_d1m_mhc_off.7z");
    ...
    // Ключевая проверка: заголовок НЕ EncodedHeader.
    ...
    Assert.DoesNotContain(folder.Coders, c => IsCopy(c.MethodId));
