...
public sealed class SevenZipReal7zBcjX86PpmdTests
{
  [Fact]
  public void DecodeToArray_Real7z_7Zip_BcjX86_PPMd_Ok()
  {
    byte[] archive = ReadTestDataBytes("../TestData/Real/bcj_x86_ppmd_mhc.7z");

    // 1) Проверяем, что в header реально есть BCJ + PPMd.
    ...
    // В этом реальном архиве NextHeader оказался несжатым (Header).
    // Для теста важно, что цепочка BCJ(x86)+PPMd корректно читается и распаковывается,
    // поэтому допускаем оба варианта (Header/EncodedHeader).
    Assert.True(
    ...
    // Параноидально проверим свойства PPMd (5 байт: order + memSize LE).
    var ppmdCoder = Array.Find(folder.Coders, c => IsPpmd(c.MethodId));
    Assert.NotNull(ppmdCoder!.Properties);
    Assert.Equal(5, ppmdCoder.Properties!.Length);

    // PPMd (7z) = PPMd7: order в диапазоне 2..64, mem >= 1<<11 (см. Ppmd7.h из SDK).
    byte order = ppmdCoder.Properties[0];
    Assert.InRange(order, (byte)2, (byte)64);

    uint mem = BinaryPrimitives.ReadUInt32LittleEndian(ppmdCoder.Properties.AsSpan(1, 4));
    Assert.True(mem >= (1u << 11));

    // У нас текущая managed-реализация/обвязка использует int под mem-size (ограничение).
    ...
    Assert.Single(files);
    Assert.EndsWith("x86_ppmd.bin", files[0].Name, StringComparison.Ordinal);
