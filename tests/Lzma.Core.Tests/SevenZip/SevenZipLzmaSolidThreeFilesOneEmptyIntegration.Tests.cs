...
public sealed class SevenZipLzmaSolidThreeFilesOneEmptyIntegrationTests
{
  [Fact]
  public void DecodeToArray_Lzma1Solid_ThreeFiles_MiddleEmptyStream_SplitBySubStreamsInfoSize_Ok()
  {
    const string name0 = "a.bin";
    const string name1 = "empty.bin";
    ...
    // Solid-поток содержит только НЕ-пустые файлы: file0 + file2.
    byte[] solid = new byte[file0.Length + file2.Length];
    Buffer.BlockCopy(file0, 0, solid, 0, file0.Length);
    Buffer.BlockCopy(file2, 0, solid, file0.Length, file2.Length);
    ...
    byte[] archive = Build7z_SolidLzma1_ThreeFiles_OneEmptyMiddle(
  ...
  private static byte[] Build7z_SolidLzma1_ThreeFiles_OneEmptyMiddle(
    byte[] packedStream,
    int unpackSizeTotal,
    ...
    int dictionarySize)
  {
    byte[] nextHeader = BuildNextHeader_SolidLzma1_ThreeFiles_OneEmptyMiddle(
  ...
  private static byte[] BuildNextHeader_SolidLzma1_ThreeFiles_OneEmptyMiddle(
    int packSize,
    int unpackSizeTotal,
    int firstNonEmptySize,
    string fileName0,
    ...
    int dictionarySize)
  {
    ...
    // EmptyStream: 3 бита, средний файл пустой => 0100_0000 (MSB->LSB).
