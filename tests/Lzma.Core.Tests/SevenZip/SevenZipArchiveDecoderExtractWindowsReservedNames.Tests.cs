...
public sealed class SevenZipArchiveDecoderExtractWindowsReservedNamesTests
{
  public static IEnumerable<object[]> InvalidWindowsEntryNames()
  {
    yield return ["CON"];
    yield return ["nul.txt"];
    yield return ["dir/PRN.bin"];
    yield return ["COM1"];
    yield return ["LPT9.txt"];

    // Хвостовой пробел/точка в последнем сегменте.
    yield return ["name "];
    yield return ["name."];

    // Хвостовой пробел/точка во внутреннем сегменте.
    yield return ["dir /file.bin"];
    yield return ["dir./file.bin"];
  }

  [Theory]
  [MemberData(nameof(InvalidWindowsEntryNames))]
  public void ExtractToDirectory_WindowsReservedOrTrimmedName_InvalidData(string entryName)
  {
    // Эти правила относятся именно к Windows.
    ...
    string root = Path.Combine(
        Path.GetTempPath(),
        "LzmaSharpTests",
        nameof(SevenZipArchiveDecoderExtractWindowsReservedNamesTests),
    ...
    try
    {
      ...
      // Корневая папка может быть создана заранее, но внутрь ничего попадать не должно.
