...
public sealed class SevenZipReal7zLzmaSolidMultiFileHeaderNotEncodedTests
{
  [Fact]
  public void DecodeToArray_Real7z_Lzma_Solid_HeaderNotEncoded_Ok()
  {
    byte[] archive = ReadTestDataBytes("TestData/Real/solid_a_empty_b_lzma_d1m_mhc_off.7z");
    ...
    Assert.True(IsLzma(folder.Coders[0].MethodId));
    Assert.False(IsLzma2(folder.Coders[0].MethodId));
    Assert.False(IsCopy(folder.Coders[0].MethodId));

    // Для 7z LZMA properties должны быть 5 байт.
  ...
  private static bool IsCopy(byte[] methodId)
      => methodId.Length == 1 && methodId[0] == 0x00;
