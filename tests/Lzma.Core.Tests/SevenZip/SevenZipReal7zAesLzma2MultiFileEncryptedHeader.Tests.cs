...
public sealed class SevenZipReal7zAesLzma2MultiFileEncryptedHeaderTests
{
  private const string Password = "LzmaSharp-AES-Stage15";

  [Fact]
  public void DecodeToEntries_РеальныйAesLzma2MultiFileАрхивСЗашифрованнымHeader_СПаролем_ВозвращаетФайлы()
  {
    byte[] archive = ReadTestDataBytes("TestData/Real/aes_lzma2_multifile_pwd_mhe_on.7z");
  ...
  [Fact]
  public void DecodeToEntries_РеальныйAesLzma2MultiFileАрхивСЗашифрованнымHeader_БезПароля_ВозвращаетNotSupported()
  {
    byte[] archive = ReadTestDataBytes("TestData/Real/aes_lzma2_multifile_pwd_mhe_on.7z");
  ...
  [Fact]
  public void DecodeToEntries_РеальныйAesLzma2MultiFileАрхивСЗашифрованнымHeader_СНевернымПаролем_ВозвращаетInvalidData()
  {
    byte[] archive = ReadTestDataBytes("TestData/Real/aes_lzma2_multifile_pwd_mhe_on.7z");
  ...
  [Fact]
  public void ExtractToDirectory_РеальныйAesLzma2MultiFileАрхивСЗашифрованнымHeader_СПаролем_ЗаписываетФайлы()
  {
    byte[] archive = ReadTestDataBytes("TestData/Real/aes_lzma2_multifile_pwd_mhe_on.7z");
  ...
  [Fact]
  public void ExtractToDirectory_РеальныйAesLzma2MultiFileАрхивСЗашифрованнымHeader_БезПароля_ВозвращаетNotSupportedИНичегоНеПишет()
  {
    byte[] archive = ReadTestDataBytes("TestData/Real/aes_lzma2_multifile_pwd_mhe_on.7z");
  ...
  [Fact]
  public void ExtractToDirectory_РеальныйAesLzma2MultiFileАрхивСЗашифрованнымHeader_СНевернымПаролем_ВозвращаетInvalidDataИНичегоНеПишет()
  {
    byte[] archive = ReadTestDataBytes("TestData/Real/aes_lzma2_multifile_pwd_mhe_on.7z");
  ...
  private static Dictionary<string, byte[]> CreateExpectedFiles()
  {
    ...
    {
      ["file-one.txt"] = Encoding.UTF8.GetBytes("LzmaSharp AES encrypted header multi-file test: file one\r\n"),
  ...
  private static string CreateTempRoot()
  {
    return Path.Combine(
        Path.GetTempPath(),
        "LzmaSharpTests",
        nameof(SevenZipReal7zAesLzma2MultiFileEncryptedHeaderTests),
