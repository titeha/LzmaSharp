...
public sealed class SevenZipReal7zAesLzma2Tests
{
  private const string Password = "LzmaSharp-AES-Stage15";

  [Fact]
  public void DecodeSingleFileToArray_РеальныйAesLzma2Архив_СПаролем_ВозвращаетИсходныйФайл()
  {
    byte[] archive = ReadTestDataBytes("../TestData/Real/aes_lzma2_singlefile_pwd_mhe_off.7z");
    ...
    Assert.Equal("aes-lzma2-real.txt", fileName);
    Assert.Equal(CreateExpectedBytes(), fileBytes);
  }

  [Fact]
  public void DecodeSingleFileToArray_РеальныйAesLzma2Архив_БезПароля_ВозвращаетNotSupported()
  {
    byte[] archive = ReadTestDataBytes("../TestData/Real/aes_lzma2_singlefile_pwd_mhe_off.7z");
  ...
  [Fact]
  public void DecodeSingleFileToArray_РеальныйAesLzma2Архив_СНевернымПаролем_ВозвращаетInvalidData()
  {
    byte[] archive = ReadTestDataBytes("../TestData/Real/aes_lzma2_singlefile_pwd_mhe_off.7z");
  ...
  [Fact]
  public void ExtractToDirectory_РеальныйAesLzma2Архив_СПаролем_ЗаписываетИсходныйФайл()
  {
    byte[] archive = ReadTestDataBytes("../TestData/Real/aes_lzma2_singlefile_pwd_mhe_off.7z");
    ...
    try
    {
      ...
      string filePath = Path.Combine(root, "aes-lzma2-real.txt");
  ...
  [Fact]
  public void ExtractToDirectory_РеальныйAesLzma2Архив_БезПароля_ВозвращаетNotSupportedИНичегоНеПишет()
  {
    byte[] archive = ReadTestDataBytes("../TestData/Real/aes_lzma2_singlefile_pwd_mhe_off.7z");
    ...
    try
    {
      ...
      Assert.False(File.Exists(Path.Combine(root, "aes-lzma2-real.txt")));
  ...
  [Fact]
  public void ExtractToDirectory_РеальныйAesLzma2Архив_СНевернымПаролем_ВозвращаетInvalidDataИНичегоНеПишет()
  {
    byte[] archive = ReadTestDataBytes("../TestData/Real/aes_lzma2_singlefile_pwd_mhe_off.7z");
  ...
  private static string CreateTempRoot()
  {
    return Path.Combine(
        Path.GetTempPath(),
        "LzmaSharpTests",
        nameof(SevenZipReal7zAesLzma2Tests),
