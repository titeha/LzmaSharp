...
public sealed class SevenZipArchiveDecoderGostKuznyechikCopyTests
{
  [Fact]
  public void DecodeSingleFileToArray_GostKuznyechikCopyArchive_СПаролем_ВозвращаетИсходныеБайты()
  {
    ...
    Assert.Equal("gost.bin", decodedName);
    Assert.Equal(plain, decoded);
  }

  [Fact]
  public void DecodeSingleFileToArray_GostKuznyechikCopyArchive_БезПароля_ВозвращаетNotSupported()
  {
    byte[] plain = System.Text.Encoding.UTF8.GetBytes(
        "LzmaSharp GOST Kuznyechik archive-level test\r\n");
  ...
  [Fact]
  public void DecodeSingleFileToArray_GostKuznyechikCopyArchive_СНевернымПаролем_ВозвращаетInvalidData()
  {
    byte[] plain = System.Text.Encoding.UTF8.GetBytes(
        "LzmaSharp GOST Kuznyechik archive-level test\r\n");
  ...
  private static byte[] Build7zArchive_SingleFile_GostKuznyechikThenCopy(
      byte[] packedStreams,
      string fileName,
      byte[] gostProperties,
      ...
      uint? folderCrc)
  {
    byte[] nextHeader = BuildHeaderSingleFolderGostKuznyechikThenCopy(
  ...
  private static byte[] BuildHeaderSingleFolderGostKuznyechikThenCopy(
      ulong packSize,
      byte[] gostProperties,
      ulong gostUnpackSize,
      ulong finalUnpackSize,
      ...
      uint? folderCrc)
  {
    ...
    WriteUnpackInfo(output, gostProperties, gostUnpackSize, finalUnpackSize, folderCrc);
      ...
      uint? folderCrc)
  {
    ...
    WriteFolderGostKuznyechikThenCopy(output, gostProperties);
