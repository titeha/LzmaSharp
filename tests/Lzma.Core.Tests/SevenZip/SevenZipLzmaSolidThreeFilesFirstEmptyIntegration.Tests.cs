...
public sealed class SevenZipLzmaSolidThreeFilesFirstEmptyIntegrationTests
{
  [Fact]
  public void DecodeToArray_Lzma1Solid_ThreeFiles_FirstEmptyStream_SplitBySubStreamsInfoSize_Ok()
  {
    const string name0 = "empty.bin";
    const string name1 = "a.bin";
    const string name2 = "b.bin";

    const int dictionarySize = 1 << 20;

    byte[] file1 = new byte[123];
    for (int i = 0; i < file1.Length; i++)
      file1[i] = (byte)(i * 17 + 3);

    byte[] file2 = new byte[77];
    for (int i = 0; i < file2.Length; i++)
      file2[i] = (byte)(i * 31 + 7);

    // Solid-поток содержит только НЕ-пустые файлы: file1 + file2.
    byte[] solid = new byte[file1.Length + file2.Length];
    Buffer.BlockCopy(file1, 0, solid, 0, file1.Length);
    Buffer.BlockCopy(file2, 0, solid, file1.Length, file2.Length);
    ...
    byte[] archive = Build7z_SolidLzma1_ThreeFiles_FirstEmpty(
      packedStream: packedStream,
      unpackSizeTotal: solid.Length,
      firstNonEmptySize: file1.Length,
      fileName0: name0,
      fileName1: name1,
      fileName2: name2,
  ...
  private static byte[] Build7z_SolidLzma1_ThreeFiles_FirstEmpty(
    byte[] packedStream,
    int unpackSizeTotal,
    ...
    int dictionarySize)
  {
    byte[] nextHeader = BuildNextHeader_SolidLzma1_ThreeFiles_FirstEmpty(
  ...
  private static byte[] BuildNextHeader_SolidLzma1_ThreeFiles_FirstEmpty(
    int packSize,
    int unpackSizeTotal,
    int firstNonEmptySize,
    string fileName0,
    ...
    int dictionarySize)
  {
    ...
    // EmptyStream: 3 бита, первый файл пустой => 1000_0000 (MSB->LSB).
